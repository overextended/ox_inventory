...
local function registerCommands()
    if client.keys[1] == client.keys[2] then
        ...
        RegisterCommand('inv', function()
            if not invOpen then
                if invBusy then return Utils.Notify({type = 'error', text = shared.locale('inventory_player_access'), duration = 2500})
                    ...
                    elseif vehicle and Vehicles.trunk['models'][vehHash] or Vehicles.trunk[class] and #(playerCoords - position) < 3 and NetworkGetEntityIsNetworked(vehicle) then
                    ...
                    elseif vehicle and #(playerCoords - position) > 3 then
                        OpenInventory()
                    ...
                    elseif closestMarker[1] and closestMarker[3] ~= 'license' and closestMarker[3] ~= 'policeevidence' then
