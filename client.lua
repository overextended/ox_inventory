			...
			if not canOpenInventory() then
				...
				if currentInventory then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id or (shared.distancecheck and #(playerCoords - pedCoords) > 1.8) or not (client.hasGroup(shared.police) or canOpenTarget(ped)) then
							client.closeInventory()
					...
					elseif currentInventory.coords and (shared.distancecheck and (#(playerCoords - currentInventory.coords) > (currentInventory.distance or 2.0)) or canOpenTarget(playerPed)) then
						client.closeInventory()
						lib.notify({ id = 'inventory_lost_access', type = 'error', description = locale('inventory_lost_access') })
					end
				end
