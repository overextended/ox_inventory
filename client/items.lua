AddEventHandler('linden_inventory:burger', function(item, wait, cb)
	...
	SetTimeout(wait, function()
		if not cancelled then
			Notify({text = 'You ate a delicious burger', duration = 2500})
		end
	end)
end)
...
AddEventHandler('linden_inventory:water', function(item, wait, cb)
	...
	SetTimeout(wait, function()
		if not cancelled then
			Notify({text = 'You drank some refreshing water', duration = 2500})
		end
	end)
end)
...
AddEventHandler('linden_inventory:cola', function(item, wait, cb)
	...
	SetTimeout(wait, function()
		if not cancelled then
			Notify({text = 'You drank some delicious eCola', duration = 2500})
		end
	end)
end)
...
AddEventHandler('linden_inventory:mustard', function(item, wait, cb)
	...
	SetTimeout(wait, function()
		if not cancelled then
			Notify({text = 'You.. drank mustard', duration = 2500})
		end
	end)
end)
	...
	if health < maxHealth then
		...
		SetTimeout(wait, function()
			if not cancelled then
				...
				Notify({text = 'You feel better already', duration = 2500})
			end
		end)
	else cb(false) end
end)
		...
		if #(vehicleCoords - GetEntityCoords(ESX.PlayerData.ped)) <= 2.0 and locked == 2 or locked == 3 then
			...
			SetTimeout(wait, function()
				if not cancelled then
					...
					print(random)
					if random >= 7 then
						Notify({text = 'Successfuly picked the lock', duration = 2500})
						SetVehicleAlarm(vehicle, true)
						SetVehicleAlarmTimeLeft(vehicle, 7000)
						SetVehicleDoorsLocked(vehicle, 1)
						SetVehicleDoorsLockedForAllPlayers(vehicle, false)
					elseif random <= 2 then
						Notify({type = 'error', text = 'Your lockpick broke', duration = 2500})
						TriggerServerEvent('linden_inventory:removeItem', item, 1)
					else
						Notify({type = 'error', text = 'You failed to pick the lock', duration = 2500})
					end
				end
			end)
		else cb(false) end
