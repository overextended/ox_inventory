	...
	if item.weapon then
		...
		elseif not next(metadata) then
			if item.name == 'identification' then
                -- The identification item would always show "male" so I had to fix it
                local genderText
				if inv.player.sex == "f" then
					genderText = "female"
				else
					genderText = "male"
				end
				...
				metadata = {
					...
					description = locale('identification', locale(genderText), inv.player.dateofbirth)
				}
