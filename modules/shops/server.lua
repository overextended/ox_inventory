			...
			for j = 1, Shops[shopName][i].slots do
				...
				if Item then
					slot = {
						...
						weight = Item.weight,
						count = slot.count,
						price = (server.randomprices and not slot.fixedPrice) and (math.ceil(slot.price * (math.random(80, 120)/100))) or slot.price,
		...
		for i = 1, Shops[shopName].slots do
			...
			if Item then
				slot = {
					...
					weight = Item.weight,
					count = slot.count,
					price = (server.randomprices and not slot.fixedPrice) and (math.ceil(slot.price * (math.random(90, 110)/100))) or slot.price,
					metadata = slot.metadata,
					license = slot.license,
					currency = slot.currency,
					grade = slot.grade
