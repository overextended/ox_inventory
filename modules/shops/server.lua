...
for shopName, shopDetails in pairs(Shops) do
	M[shopName] = {}
	M[shopName].label = shopDetails.name
	M[shopName].job = shopDetails.job
	M[shopName].items = table.clone(shopDetails.inventory)
	M[shopName].slots = #shopDetails.inventory
