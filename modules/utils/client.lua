...
function Utils.Raycast()
	...
	local rayHandle = StartShapeTestCapsule(playerCoords.x, playerCoords.y, playerCoords.z, plyOffset.x, plyOffset.y, plyOffset.z, 2.2, 12, PlayerData.ped)
	while true do
		Wait(0)
...
function Utils.Disarm(currentWeapon, newSlot)
	...
	if currentWeapon then
		...
		if not newSlot then
			...
			local sleep = (client.isPolice() and (GetWeapontypeGroup(currentWeapon.hash) == 416676503 or GetWeapontypeGroup(currentWeapon.hash) == 690389602)) and 450 or 1400
