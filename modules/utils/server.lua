...
-- Serpent version 0.303 (C) 2012-18 Paul Kulchenko; MIT License
...
---Serializes a table, prettifying and optimizing certain aspects such as removing [""] around keys,
---or removing explicit keys from an array. Uses Serpent as the underlying library.
---Does not work for functions within tables.
---@param table table
---@param keyOrder string[] the table keys in the order they should appear in the output
---@return string serialized
function Utils.TableToString(table, keyOrder)
	return s(table, merge({
		indent = '  ',
		sortkeys = function(sortTable)
			table.sort(sortTable, function(one, two)
				local oneI = 0
				local twoI = 0
				for i=1, #keyOrder do
					if keyOrder[i] == one then
						oneI = i
					end
					if keyOrder[i] == two then
						twoI = i
					end
