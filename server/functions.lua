	...
	if data.type ~= 'create' then
		...
		if data.type == 'freeslot' then
			if ValidateItem(data.type, xPlayer, Inventories[invid2].inventory[data.emptyslot], Drops[invid].inventory[data.toSlot], data.item, data.item) == true then
				...
				Drops[invid].inventory[data.toSlot] = {name = data.item.name, label = data.item.label, weight = data.item.weight, slot = data.toSlot, count = data.item.count, description = data.item.description, metadata = data.item.metadata, stackable = data.item.stacks, closeonuse = Items[data.item.name].closeonuse}
				if Config.Logs then
		...
		for k,v in pairs(data.inventory) do
			...
			if xItem then
				...
				Drops[invid].inventory[k] = {name = v.name , label = xItem.label, weight = xItem.weight, slot = v.slot, count = v.count, description = xItem.description, metadata = v.metadata, stackable = xItem.stacks,  closeonuse = xItem.closeonuse}
			end
		end
...
SetupShopItems = function(shop)
	...
	for k, v in pairs(Config.Shops[shop].store.inventory) do
		...
		if xItem then
			if v.metadata == nil then v.metadata = {} end
			inventory[k] = {name = v.name, label = xItem.label, weight = xItem.weight, slot = k, count = v.count, description = xItem.description, metadata = v.metadata, stackable = xItem.stacks, price = v.price}
		end
	end
	return inventory
end
		...
		if result ~= nil then
			...
			for k,v in pairs(Inventory) do
				if Items[v.name] then
					if v.metadata == nil then v.metadata = {} end
					returnData[v.slot] = {name = v.name , label = Items[v.name].label, weight = Items[v.name].weight, slot = v.slot, count = v.count, description = Items[v.name].description, metadata = v.metadata, stackable = Items[v.name].stacks, closeonuse = Items[v.name].closeonuse}
				end
			end
		end
	end
