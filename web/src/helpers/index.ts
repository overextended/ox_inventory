...
export const isSlotWithItem = (slot: Slot, strict: boolean = false): slot is SlotWithItem =>
    (slot.name !== undefined && slot.weight !== undefined) || (slot.name == 'clothing') ||
...
export const findAvailableSlot = (item: Slot, data: ItemData, items: Slot[]) => {
    ...
    // console.log(`${item.name === 'clothing'}`);
    // console.log(`${items.find((target) => target.name === 'clothingslot')}`);
    // console.log(`${items.find((target) => target.name === 'clothingslot' && item.name === 'clothing' && (target.metadata?.clothingslot?.id === item.metadata?.component))}`);
    if (items.find((target) => target.name === 'clothingslot' && item.name === 'clothing' && (target.metadata?.clothingslot?.id === item.metadata?.component))) {
        return items.find((target) => target.name === 'clothingslot' && item.name === 'clothing' && (target.metadata?.clothingslot?.id === item.metadata?.component));
    }
    ...
    return stackableSlot || items.find((target) => target.name === 'clothingslot' && item.name === 'clothing' && (target.metadata?.clothingslot?.id === item.metadata?.component)) || items.find((target) => target.name === undefined);
};
